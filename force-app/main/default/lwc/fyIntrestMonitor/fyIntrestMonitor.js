...
export default class FyIntrestMonitor extends NavigationMixin(
  LightningElement
) {
  subscription = null;
  ...
  @wire(interestWrapper, { fy: "$fyvalue", entityName: "$entityName" })
  ...
  subscribeToMessageChannel() {
    ...
    if (!this.entitySubscription) {
      this.entitySubscription = subscribe(
        this.messageContext,
        ENTITYCHANGE_CHANNEL,
        (message) => this.handleEntityMessage(message),
        { scope: APPLICATION_SCOPE }
      );
    }
  }
  ...
  unsubscribeToMessageChannel() {
    ...
    unsubscribe(this.entitySubscription);
  ...
  get allInvestments() {
    if (!isValid(this.investmentMap)) {
      ...
      log(
        "inside getter of Investments***** : " +
          this.investmentMap[this.selectedBank]
      );
  ...
  calculateTotals() {
    ...
    this.totalAccumulated =
      this.totalAccumulated > 0 ? this.totalAccumulated.toFixed(2) : 0;
